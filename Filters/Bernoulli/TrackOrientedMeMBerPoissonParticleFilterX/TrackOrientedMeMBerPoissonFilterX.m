classdef TrackOrientedMeMBerPoissonFilterX < FilterX
% TrackOrientedMeMBerPoissonFilterX class
%
% Summary of TrackOrientedMeMBerPoissonFilterX:
% This is a class implementation of a Track-Oriented Marginal Hybrid 
% Multi-Bernoulli (MeMBer) Poisson Filter [1].
%
% TrackOrientedMeMBerPoissonFilterX Properties:
...
%
% TrackOrientedMeMBerPoissonFilterX Methods:
%   + TrackOrientedMeMBerPoissonFilterX - Constructor method
...
% See also ParticleFilterX, KalmanFilerX.
    ...
    methods
        function this = TrackOrientedMeMBerPoissonFilterX(varargin)
        % TrackOrientedMeMBerPoissonFilterX - Constructor method
        %   
        % Parameters
        % ----------
        ...
        % Usage
        % -----
        % * phd = TrackOrientedMeMBerPoissonFilterX() returns an unconfigured object 
        %   handle. Note that the object will need to be configured at a 
        %   later instance before any call is made to it's methods.
        % * phd = TrackOrientedMeMBerPoissonFilterX(ssm) returns an object handle,
        %   preconfigured with the provided StateSpaceModelX object handle ssm.
        % * phd = TrackOrientedMeMBerPoissonFilterX(ssm,priorParticles,priorWeights) 
        ...
        % * phd = TrackOrientedMeMBerPoissonFilterX(ssm,priorDistFcn) returns an object
        ...
        % * phd = TrackOrientedMeMBerPoissonFilterX(___,Name,Value,___) instantiates an  
        ...
        % Usage
        % ----- 
        % * initialise(phd,ssm) initialises the TrackOrientedMeMBerPoissonFilterX object 
        %   phd with the provided StateSpaceModelX object ssm.
        % * initialise(phd,priorParticles,priorWeights)initialises the 
        %   TrackOrientedMeMBerPoissonFilterX object pf with the provided StateSpaceModel     
        ...
        % * initialise(phd,ssm,priorDistFcn) initialises the TrackOrientedMeMBerPoissonFilterX
        ...
        % More details
        % ------------
        % * TrackOrientedMeMBerPoissonFilterX uses the Model class property, which should 
        ...
        %  See also update, smooth.
            ...
            for i = 1:numel(this.Bernoulli.Updated)
                ...
                this.Bernoulli.Predicted{i}.ProbOfExistence = ...
                    this.ProbOfSurvive * this.Bernoulli.Updated{i}.ProbOfExistence;
        ...
        % See also UnscentedParticleFilterX, predict, smooth.
            ...
            for i = 1:numel(this.Bernoulli.Predicted)
                ...
                this.Bernoulli.Updated{i}.WeightsPerHypothesis(1,:) =  (1-this.Bernoulli.Predicted{i}.ProbOfExistence ...
                  + this.Bernoulli.Predicted{i}.ProbOfExistence*(1-this.ProbOfDetection)) * this.Bernoulli.Predicted{i}.Weights;
                this.Bernoulli.Updated{i}.ExistencePerHypothesis(1) = ...
                    this.Bernoulli.Predicted{i}.ProbOfExistence*(1-this.ProbOfDetection)...
                    /(1-this.Bernoulli.Predicted{i}.ProbOfExistence + this.Bernoulli.Predicted{i}.ProbOfExistence*(1-this.ProbOfDetection));
                
                % Evaluate hypotheses with measurement updates
                this.Bernoulli.Updated{i}.WeightsPerHypothesis(2:end,:) = ...
                    this.Bernoulli.Predicted{i}.ProbOfExistence * this.ProbOfDetection * g .* this.Bernoulli.Predicted{i}.Weights;
                this.Bernoulli.Updated{i}.ExistencePerHypothesis(2:end) = 1;
                
                % Update Assiciation likelihoods
                this.AssocLikelihoodMatrix(i+1,:) = sum(this.Bernoulli.Updated{i}.WeightsPerHypothesis,2)';
            ...
            PoissonWeightsPerMeasurement = zeros(this.NumMeasurements, size(this.Poisson.Updated.Particles,2));
            if(this.NumMeasurements>0)
                PoissonWeightsPerMeasurement = (this.ProbOfDetection*g./(repmat(this.ClutterRate*this.ClutterIntFcn(this.MeasurementList)',1,size(g,2))+Ck)).*this.Poisson.Predicted.Weights;
                %PoissonWeightsPerMeasurement = (Ck+repmat(this.ClutterRate*this.ClutterIntFcn(this.MeasurementList)',1,size(this.Poisson.Predicted.Weights,2))).*this.Poisson.Predicted.Weights;
            end
            ...
            for j = 1:this.NumMeasurements
                ...
                [newTrack.Particles,newTrack.Weights,idx] = this.Resampler.resample(newTrack.Particles,newTrack.Weights,1000);
                
                Ck_j = Ck(j,idx);
                C = sum(Ck_j,2);
                
                % Compute weights per hypothesis
                AssocLikelihoodPerParticle = (Ck_j+ this.ClutterRate*this.ClutterIntFcn(this.MeasurementList(:,j))).*newTrack.Weights;
                this.AssocLikelihoodMatrix(1,j+1) = sum(AssocLikelihoodPerParticle,2)';
                
                % Compute probability of existence
                newTrack.ProbOfExistence = C/(C+this.ClutterRate*this.ClutterIntFcn(this.MeasurementList(:,j)));
            ...
            [this.Poisson.Updated.Particles,this.Poisson.Updated.Weights] = ...
                this.Resampler.resample(this.Poisson.Updated.Particles, ...
                    (this.Poisson.Updated.Weights/NumPPPTargets),10000); % Resample
            ...
            for i = 1 : numel(this.Bernoulli.Predicted)
                ...
                [this.Bernoulli.Updated{i}.Particles,this.Bernoulli.Updated{i}.Weights] = ...
                    this.Resampler.resample(this.Bernoulli.Updated{i}.Particles, this.Bernoulli.Updated{i}.Weights,1000); 
            ...
            this.Bernoulli.Updated = this.Bernoulli.Updated(find(cell2mat(cellfun(@(c) c.ProbOfExistence, this.Bernoulli.Updated, 'UniformOutput', false))>10^(-2)));
