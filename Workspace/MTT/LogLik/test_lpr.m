...
%% Clutter settings
%lambdaV = 400; % mean number of clutter points 
...
% Params_kf.P_init = diag([Params_po.r^2, Params_po.r^2,10,10]);
...
Params.Np              = 20000;              % number of particles
...
if(exist('exec_time_lpr','var'))
    exec_time_lpr(:,end+1) = zeros(2,1);
else
    exec_time_lpr = zeros(2,1);
end
for i = 1:N
    tic;
    fprintf('\nIteraration: %d/%d\n', i, N);
...
%     end
    ...
    THD  = -8.4;
    t2 = tic;
    [jpdaf, pdaf, TrackIds] =  LogLikRatio_TM(jpdaf, pdaf, [0.1,0.04,THD], initFilter, TrackIds);
    exec_time_lpr(1,end) = exec_time_lpr(1,end) + toc(t2);
    exec_time_lpr(2,end) = exec_time_lpr(2,end) + toc(t1);
    ...
    for j=1:jpdaf.Params.nTracks
        ...
        Logs{t}.LPR_diffV(i) = jpdaf.Params.TrackList{j}.LPR - jpdaf.Params.TrackList{j}.LPR_max - THD;
    end
...
%                 end
                ...
                if(~isnan(Logs{j}.xV(1,i)))
                    ...
                    text(ax(1),Logs{j}.xV(1,i)+0.25,Logs{j}.xV(2,i)-.35,num2str(Logs{j}.LPR_diffV(i), 2));
                end
            end
...
if(exist('ospa_vals_lpr','var'))
    ospa_vals_lpr(:,:,end+1)= zeros(N,3);
else
    ospa_vals_lpr(:,:)= zeros(N,3);
end
...
for k=1:N
    ...
    [ospa_vals_lpr(k,1,end), ospa_vals_lpr(k,2,end), ospa_vals_lpr(k,3,end)]= ospa_dist(trueX,estX,ospa_c,ospa_p);
end

figure; ospa= gcf; hold on;
subplot(3,1,1); plot(1:N,ospa_vals_lpr(:,1,end),'k'); grid on; set(gca, 'XLim',[1 N]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Dist');
subplot(3,1,2); plot(1:N,ospa_vals_lpr(:,2,end),'k'); grid on; set(gca, 'XLim',[1 N]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Loc');
subplot(3,1,3); plot(1:N,ospa_vals_lpr(:,3,end),'k'); grid on; set(gca, 'XLim',[1 N]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Card');
xlabel('Time');

% x_1(1:134,:) = [x1(1:134,1),y1(1:134,1)];
% x_1(135:1184,:) = NaN(1184-134,2);
% x_2(1:175,:) = NaN(175,2);
% x_2(176:end,1) = x1(176:end,1);
% x_2(176:1184,1) = x1(176:end,1);
% x_2(176:1184,2) = y1(176:end,1);
% x_3(1:338,:) = [x1(1:338,1),y1(1:338,1)];
% x_3(339:1184,:) = NaN(1184-338,2);
% x_4(1:380,:) = NaN(380,2);
% x_4(381:728,1) = x1(381:728,2);
% x_4(381:728,2) = y1(381:728,2);
% x_4(729:1184,:) = NaN(1184-728,2);
% x_5(1:769,:) = NaN(769,2);
% x_5(770:986,1) = x1(770:986,2);
% x_5(770:986,2) = y1(770:986,2);
% x_5(771:1184,:) = NaN(1184-770,2);
% x_6(1:1046,:) = NaN(1046,2);
% x_6(1047:N,1) = x1(1047:end,2);
% x_6(1047:N,2) = y1(1047:end,2);
% x_7(1:264,1) = x1(1:264,3);
% x_7(1:264,2) = y1(1:264,3);
% x_7(265:N,:) = NaN(N-264,2);
% x_8(1:306,:) = NaN(306,2);
% x_8(307:431,1) = x1(307:431,3);
% x_8(307:431,2) = y1(307:431,3);
% x_8(432:N,:) = NaN(N-431,2);
% x_9(1:471,:) = NaN(471,2);
% x_9(472:629,1) = x1(472:629,3);
% x_9(472:629,2) = y1(472:629,3);
% x_9(630:N,:) = NaN(N-629,2);
% x_10(1:788,:) = NaN(788,2);
% x_10(789:N,1) = x1(789:end,3);
% x_10(789:N,2) = y1(789:end,3);
% 
% F = F(2:end);
...
% Logs_true{8}.xV = x_8;
% Logs_true{9}.xV = x_9;
% Logs_true{10}.xV = x_10;
% figure
% for i = 1:10
%     plot(Logs_true{i}.xV(:,1), Logs_true{i}.xV(:,2));
%     hold on;
% end
