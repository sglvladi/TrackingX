...
for cellInd = 1:length(GroundTruth)
    GroundTruth{cellInd} = GroundTruth{cellInd}.*100;
end
x_true = x_true.*100;
y_true = y_true.*100;
...
dyn = ConstantHeadingModelX('VelocityErrVariance',(3)^2, 'HeadingErrVariance',0.16);
...
% Generate DataList
%[DataList,~,inds] = dataGen(obs,GroundTruth,3,100,[],[],1);
...
for i=1:NumTracks
    ...
    Params_kf.PriorStateMean = [GroundTruth{1}(1,i); GroundTruth{1}(2,i); 0; 0];
    ...
    Params_pf.Model = ssm;
    Params_pf.NumParticles = 10000;
    Params_pf.PriorParticles = mvnrnd([GroundTruth{1}(1,i); GroundTruth{1}(2,i); 0; 0]', dyn.covariance(), Params_pf.NumParticles)';
...
Params_jpdaf.Gater = EllipsoidalGaterX(2,'GateLevel',15)';
    ...
    if (ShowPlots)
        if(i==1 || rem(i,SkipFrames+1)==0)
            ...
            for j=1:NumTracks
                ...
                h3 = plot(Logs{j}.Estimates.StateMean(1,1:i),Logs{j}.Estimates.StateMean(2,1:i),'r.-','LineWidth',1);
                h4=plot_gaussian_ellipsoid(Logs{j}.Estimates.StateMean([1 2],i), Logs{j}.Estimates.StateCovar([1 2],[1 2],i));
            end
            meas2state = obs.heval_inv([DataList{i}(1,:);DataList{i}(2,:)]);
            h2 = plot(meas2state(1,:),meas2state(2,:),'k*','MarkerSize', 10);
...
%             figure(2);
%             clf;
%             Xk = [jpdaf.TrackList{1}.Filter.Particles(1:2,:), jpdaf.TrackList{2}.Filter.Particles(1:2,:), jpdaf.TrackList{3}.Filter.Particles(1:2,:)];
...
for k=1:N
    ...
    for i=1:NumTracks
        estX(:,i) = Logs{i}.Estimates.StateMean([1 2],k);
    end
