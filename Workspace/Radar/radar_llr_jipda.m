...
transition_model = ConstantHeadingX('VelocityErrVariance',0.1,...
...
%% Base Filter
PriorState = GaussianStateX(zeros(4,1), 50*transition_model.covar() + 10*blkdiag(obs_covar(1,1), obs_covar(2,2), 0, 0));
...
%     pause(0.01);
    ...
    [TrackList] = myti.initiateTracks(jpdaf.TrackList, MeasurementList, jpdaf.AssocWeightsMatrix, jpdaf.LikelihoodMatrix);
        ...
        for j=1:numel(TrackList)
            ...
            if PlotLatLon
                ...
                if ShowTrackInfo
                    ...
                    plots(end+1) = text(ax(1), lon(:,end)+0.001,lat(:,end)+0.00027,strcat("ID: ",num2str(track.Tag.ID)),'FontSize',8,'Color','w');
                    plots(end+1) = text(ax(1), lon(:,end)+0.001,lat(:,end)-0.00027,strcat("LLR:",num2str(track.LPR,3),""),'FontSize',8,'Color','w');
...
% 
                ...
                if ShowTrackInfo
                    ...
                    speed_knot = speed_kmph/1.852;
                    plots(end+1) = text(ax(1), means(1,end)+60,means(2,end)+50,strcat("ID: ",track.Tag.ID),'FontSize',8,'Color','k');
