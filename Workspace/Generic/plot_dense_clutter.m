...
% Load dataset
load('staddon.mat');
MeasurementScansAll = MeasurementScans;
load(strcat(RadarName,'.mat'));

lon_lim = [-4.2637   -4.1024];
lat_lim = [50.3516   50.3640];

lon_lim_2 = [-4.176846509809199,-4.103030078951917];
lat_lim_2 = [50.297532151330770,50.337022151330770];

%lon_lim_obs = [-4.1674   -4.1490];
%lat_lim_obs = [50.3516   50.3640];
lon_lim_obs = [-4.176846509809199,-4.103030078951917];
lat_lim_obs = [50.297532151330770,50.337022151330770];
...
for i = 1:2 
    ...
    if i==1
        ...
        N = size(MeasurementScansAll,2); % Simulation length

        for k=2:N
            MeasurementList = MeasurementScansAll(k);
...
%         x = [lon_lim_2(1), lon_lim_2(2), lon_lim_2(2), lon_lim_2(1), lon_lim_2(1)];
%         y = [lat_lim_2(2), lat_lim_2(2), lat_lim_2(1), lat_lim_2(1), lat_lim_2(2)];
%         plot(ax, x, y, 'w-', 'LineWidth', 2);
        pos = [-4.175308668249532,50.299082965407180,0.060353057955262,0.036240150373260];
...
%         axis(ax,[lon_lim(1) lon_lim(2) lat_lim(1) lat_lim(2)])
       ...
       % Plot region of high clutter
        ...
        rectangle(ax, 'Position', pos, ...
                      'EdgeColor', 'w', 'LineWidth', 2)
        h1 = plot(ax, NaN, NaN, 'o','MarkerSize', 5, 'MarkerFaceColor', 'y', 'MarkerEdgeColor','k');
        h2 = plot(ax, NaN, NaN, 'r-', 'LineWidth', 2);
        legend(ax, [h1, h2, h2], 'Detections', 'High-clutter Region');
%         xlabel('Longitude');
%         ylabel('Latitude');
    end
%     plot(ax, randn(10,i)); 
