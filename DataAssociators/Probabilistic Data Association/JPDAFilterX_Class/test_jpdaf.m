...
for i = 1:N
    ...
    jpdaf.Params.k = 1;
    jpdaf.Params.DataList = DataList{i}(:,:);
    jpdaf.Params.DataList( :, ~any(jpdaf.Params.DataList,1) ) = [];
    
    for j=1:TrackNum
        %Logs{j}.xV_ekf(:,i) = jpdaf.Params.TrackList{j}.TrackObj.Params.x;
        st = [x1(i,j); y1(i,j)];
        Logs{j}.sV_ekf(:,i)= st;
        %Logs{j}.eV_ekf(:,i) = (jpdaf.Params.TrackList{j}.TrackObj.Params.x(1:2,1) - st).*(jpdaf.Params.TrackList{j}.TrackObj.Params.x(1:2,1) - st);
    end
    tic;
    jpdaf.Predict();
    %exec_time = 
