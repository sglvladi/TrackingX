...
for k=2:N
    ...
    for t = 1: numel(jpdaf.TrackList)
        jpdaf.TrackList{t}.Trajectory(end+1) = jpdaf.TrackList{t}.Filter.StatePosterior;
    end
    ...
    if(ShowPlots)
        ...
        % Plot tracks
        for j=1:numel(jpdaf.TrackList)
            means = [jpdaf.TrackList{j}.Trajectory.Mean];
            h2 = plot(ax(1), means(1,:),means(3,:),'-','LineWidth',1);
            h2 = plot_gaussian_ellipsoid(jpdaf.TrackList{j}.Filter.StatePosterior.Mean([1 3]), jpdaf.TrackList{j}.Filter.StatePosterior.Covar([1 3],[1 3]),'r',1,20,ax(1)); 
        end
