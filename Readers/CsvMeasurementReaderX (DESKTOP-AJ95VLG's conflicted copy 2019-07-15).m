...
% compatibility with the TrackingX framework.
    ...
    methods (Access = protected)
        function initialise_(this, config)
            ...
            this.Table_ = sortrows(this.Table_, this.TimeField);
    ...
    methods
        ...
        function [measurements, timestamp] = read(this)
            ...
            while(time == this.TimeValues_{this.RowIndex_})
                stateVec = this.Table_{this.RowIndex_, this.StateFields};
                valid = 0;
                stateVector = [];
                for sV=stateVec
                    if ~strcmp(sV{:}, '')
                        valid = valid + 1;
                        stateVector = [stateVector; double(string(sV{:}))];
                    end
                end
                if valid ~= size(stateVector,1)
                    this.RowIndex_ = this.RowIndex_ + 1;
                    continue
                else
                    this.RowIndex_ = this.RowIndex_ + 1;
                    stateVectors = [stateVectors, double(stateVector)];
                end
