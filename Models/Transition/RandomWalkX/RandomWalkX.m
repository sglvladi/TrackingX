classdef RandomWalkX < TransitionModelX
% RandomWalkX class
%
% Summary of RandomWalkX
% This is a class implementation of a simple Random Walk Dynamic Model.
%
% The model is described by the following SDEs:
%   dx  = q*dW(t),  W(t)~N(0,t)
%
% Or equivalently:
%   x(t) = x(t-1) + w(t),  w(t)~ N(0,Q)
%
% where:
%   x = [x];
%   F = [1];
%   Q = [q^2dt];
%
% RandomWalkX Properties:
%   + ErrVariance              Value of the noise diffusion coefficient q^2
...
%  February 2018 Lyudmil Vladimirov, University of Liverpool
    ...
    methods (Access = protected)
        function initialise_(this, config)
            ...
            if (isfield(config,'ErrVariance'))
                this.ErrVariance = config.ErrVariance;
                if(this.NumDims>length(this.ErrVariance))
                    this.ErrVariance = this.ErrVariance(ones(1,this.NumDims),1);
                end
            end
                     
            this.F = eye(this.NumDims);
        end
        
        function Qk = Q(this,dt)
            Qk = [];
            for i = 1:this.NumDims
               Qk = blkdiag(Qk,this.Q_rw(dt,this.ErrVariance(i)));
            end
        end
    end
    
    methods
        function this = RandomWalkX(varargin)
        % RandomWalkX Constructor method
        %   
        % Parameters
        % ----------
        % ErrVariance: scalar or vector
        %   Describes the process noise diffusion coefficient. If NumDims
        %   is higher that 1 and ErrVariance is scalar, then the
        %   same noise coefficient will be applied to all dimensions.
        %   Otherwise, ErrVariance can be a vector of length
        ...
        % Usage
        % ----- 
        % * RandomWalkX(___,Name,Value) instantiates an object 
        %   handle, configured the parameters specified by one or
        %   more Name,Value pair arguments. 
        % * RandomWalkX(config) instantiates an object handle configured  
        ...
        function mat = matrix(this,varagin)
