classdef OrnsteinUhlenbeckModelX < TransitionModelX
% OrnsteinUhlenbeckModelX class
%
% Summary of OrnsteinUhlenbeckModelX
...
%  February 2018 Lyudmil Vladimirov, University of Liverpool
    ...
    methods (Access = protected)
        ...
        function Q = Q_ou(this,dt,q,K)
            expmKdt = exp(-K*dt);
            expm2Kdt = exp(-2*K*dt);
            q11 = q/K^2*(dt - 2/K*(1 - expmKdt) + 1/(2*K)*(1 - expm2Kdt));
            q12 = q/K*((1 - expmKdt)/K - 1/(2*K)*(1 - expm2Kdt));
            q22 = q/(2*K)*(1 - expm2Kdt);
            Q = [q11 q12; q12 q22];
        end
        ...
        function initialise_(this, config)
            ...
            if (isfield(config,'DampingCoefficient'))
                this.DampingCoefficient = config.DampingCoefficient;
                if(this.NumDims>length(this.DampingCoefficient))
                    this.DampingCoefficient = this.DampingCoefficient(ones(1,this.NumDims),1);
                end
            end
        end
        
        function Fk = F(this,dt)
            Fk = [];
            for i = 1:this.NumDims
               Fk = blkdiag(Fk, this.F_ou(dt,this.DampingCoefficient(i)));
            end
        end
        ...
        function Qk = Q(this,dt)
            Qk = [];
            for i = 1:this.NumDims
               Qk = blkdiag(Qk,this.Q_ou(dt,...
                                         this.VelocityErrVariance(i),...
    ...
    methods
        function this = OrnsteinUhlenbeckModelX(varargin)
        % OrnsteinUhlenbeckModelX Constructor method
        %   
        % Parameters
        % ----------
        ...
        %   the i-th dimension.
        % DampingCoefficient: scalar or vector
        %   Ornestein-Uhlenbeck velocity damping coefficient (K)
        % NumDims: scalar
        ...
        % Usage
        % ----- 
        % * OrnsteinUhlenbeckModelX(___,Name,Value) instantiates an object 
        %   handle, configured the parameters specified by one or
        %   more Name,Value pair arguments. 
        % * OrnsteinUhlenbeckModelX(config) instantiates an object handle configured  
